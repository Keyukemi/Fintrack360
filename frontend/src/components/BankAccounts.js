...
const AllBankAccounts = () => {
    ...
    const fetchBankAccount = async () => {
        ...
        setError('');
        try {
            const response = await axios.post('/api/bvn/bank-accounts-detail', { otp, sessionId, userId });
                    ...
                    <>
                        ...
                        {stage === 'fetch' && userBankAccounts.length === 0 && (
                            <form onSubmit={handleSubmit}>
                                ...
                                <button type="submit" className="bg-tertiary hover:bg-primary text-white font-bold py-2 px-4 rounded">Fetch Bank Account</button>
                            </form>
                        )}
                    </>
                )}
